...
- Design system (2026-09-16, from the Framer reference): Host Grotesk everywhere, background `rgb(8,10,9)`, surfaces `rgb(19,21,23)` with 1px `rgba(255,255,255,.09)` borders and 4px radii, white primary buttons, coloured pill indicators on section labels. Marketing landing at `/` (`app/(marketing)`), app shell with sidebar under `app/(app)`: `/portfolio`, `/assets`, `/assets/[assetId]` (chart from tokens.xyz `price-chart` with `from`, wrappers, venues, rights profiles), `/actions`, `/actions/[id]`, `/issuer`, `/adapters`. `/holder` redirects to `/portfolio`. Screenshots for review: `cd apps/web && node ../../scripts/dev/screenshots.mjs`.
