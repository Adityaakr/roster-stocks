...
## 2026-09-16, landing copy v3

- The landing now follows the v3 copy spec verbatim: positioning (rights or composability was a false choice), hero with the invisible-shares check as the instrument, the measured `30.35%` block printed from the visibility file without rounding, the false-choice table, the dividend-to-the-pool narrative, eight dated and linked citations, the 1973 look-through, how it works, the settlement leg, what the multiplier cannot do, the size of the job, product, audiences, timeline, what is determined by whom, questions, closing.
...
- Second pass on the same day: a scan of every component for effects beyond appear and text reveals found two more, now reproduced: the brand row is a `tickerEffect` (50 px/s, 40% on hover) and the two large use-case cards carry a `styleTransformEffect` (x ±290 to 0, spring 300/100) so they converge on view.
